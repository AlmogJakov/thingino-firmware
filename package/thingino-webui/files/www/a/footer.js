(function () {
  ...
  function ensureMessageOverlay() {
    ...
    if (!el) {
      ...
      el.className =
        globalConfig.restartMessageClass || "global-message-overlay";
  ...
  async function handleRestartClick(event) {
    ...
    try {
      if (btn) {
        btn.disabled = true;
        const originalLabel =
          btn.dataset.originalLabel || btn.innerHTML || btn.textContent;
        btn.dataset.originalLabel = originalLabel;
        btn.classList.add("pe-none");
        btn.innerHTML =
          '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span><span>' +
      ...
      if (globalConfig.restartReload === false) {
        ...
        const reloadDelay = Number.isFinite(reloadDelayCfg)
          ? reloadDelayCfg
          : 800;
        window.setTimeout(
          () => {
  ...
  function buildFooter() {
    ...
    footer.className =
      globalConfig.className || "x-small text-secondary border-top";
