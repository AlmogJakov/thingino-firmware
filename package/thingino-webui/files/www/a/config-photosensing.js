(function () {
    ...
    try {
      ...
      if (daynight.controls) {
        dayNightControls.forEach((control) => {
          ...
          if (
            !el ||
            !Object.prototype.hasOwnProperty.call(daynight.controls, control)
          )
            return;
      ...
      if (daynight.schedule) {
        dayNightScheduleParams.forEach((param) => {
          ...
          if (
            !el ||
            !Object.prototype.hasOwnProperty.call(daynight.schedule, param)
          )
            return;
          if (el.type === "checkbox") {
  ...
  async function loadPhotosensingConfig(options = {}) {
    ...
    try {
      ...
      showAlert(
        "danger",
        `Unable to load photosensing settings: ${err.message || err}`,
      );
      return false;
    } finally {
      if (!silent) {
  ...
  async function saveDaynightScheduleParam(param) {
    ...
    try {
      ...
      showAlert(
        "danger",
        `Failed to update schedule ${param.replace(/_/g, " ")}: ${err.message || err}`,
      );
    }
  }
    ...
    reloadButton.addEventListener("click", async () => {
      try {
        ...
        if (success) {
          showAlert(
            "info",
            "Photosensing settings reloaded from camera.",
            3000,
          );
        }
      } catch (err) {
